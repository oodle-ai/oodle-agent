...
```bash
helm upgrade --install oodle-agent \
  oci://public.ecr.aws/oodle-ai/helm/oodle-agent \
...
## Publishing to AWS ECR Public

### Prerequisites

- AWS CLI v2 installed and configured
- Docker installed and running
- Access to the AWS account that owns the ECR repo
...
# (ECR Public auth always uses us-east-1)
...
### Push Helm Chart to ECR Public

```bash
# Package the helm chart
helm package helm/oodle-agent
...
# Push the chart
helm push oodle-agent-*.tgz \
  oci://public.ecr.aws/${ECR_ALIAS}/helm
```

### Create ECR Public Repository (one-time)

```bash
# Create Docker image repo
aws ecr-public create-repository \
  --repository-name oodle-agent \
  --region us-east-1

# Create Helm chart repo
aws ecr-public create-repository \
  --repository-name helm/oodle-agent \
  --region us-east-1
```
